...
def generate_uri_from_put(file_name, real_file):
    ...
    # born unique no
    x = str(uuid.uuid4())+file_name
    file_hash = hashlib.md5(x).hexdigest()
    ...
    file_map = FileMap(file_name=file_hash, file_extension_name=file_extension_name, file_uri=file_uri)
